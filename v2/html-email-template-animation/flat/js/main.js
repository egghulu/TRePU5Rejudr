...
$(document).ready(function() {   
	 ...
	 $('#feature_content').slideToggle();
	$('#show_feature').click(function(event){
	...
	function click_icon_responsive(icon){
		$('#'+icon).parent().click(function(){
			...
			if($('#'+icon).attr('id') == 'icon_iphone'){
				...
				landscape = 0;	
				$('#box_iframe').animate({ width:335 ,height:585, left:$(window).width()/2-162,top:$(window).height()/2-293}, 500, "easeOutSine");
				$('#iframe').animate({ width:335 ,height:585}, 500, "easeOutSine");
				//$('#iframe').animate({ width:345 ,height:585}, 500, "easeOutSine");
				$('#box_iframe').css({
					marginTop:0
			...
			if($('#'+icon).attr('id') == 'icon_iphone_landscape'){
				...
				//$('#iframe').animate({ width:582 ,height:335}, 500, "easeOutSine");
	...
	function addRel(activeColor){
		...
		$('.screenshot').each(function(i){
			...
			if($(this).attr('id') == "TemplateBuilder"){
				...
				$(this).attr('rel','images/layout/'+activeColor+'/'+$(this).attr('id')+'.png')
			}
